...
//
    ...
    public void operatorControl() {
        ...
        while (isOperatorControl() && isEnabled()) {
            ...
            if(winchUp){//if the winch is up
                jag5.set(1);
                jag6.set(1);
            }else if(!winchUp&&released){//if winch is down and released is not running shut down the winch
                jag5.set(0);
                jag6.set(0);
            }
            ...
            }if(!released&&!winchUp){//reverses for _ seconds, because it is latched
                if( (timerI<5 && isGoingToBePass) || (timerI<10)){  //for half shots
                    jag5.set(-.7);
                    jag6.set(-.7);
                }else{
                    released=true;
    ...
    public void trigger(){
        //fail safe for full fire on ready for half
        if (passCounterFailSafe > 0){
            passCounterFailSafe += 1;
            if (passCounterFailSafe > 5){
                passCounterFailSafe = 0;
